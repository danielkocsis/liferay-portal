...
public class StagedAssetLinkStagedModelDataHandler
	...
	public void deleteStagedModel(
		...
		StagedAssetLink stagedAssetLink = fetchExistingAssetLink(
			groupId, parseEntry1Uuid(uuid), parseEntry2Uuid(uuid));
	...
	public List<StagedAssetLink> fetchStagedModelsByUuidAndCompanyId(
		...
		DynamicQuery dynamicQuery = getAssetLinkDynamicQuery(
			companyId, 0, parseEntry1Uuid(uuid), parseEntry2Uuid(uuid));
	...
	protected void doImportStagedModel(
		...
		Map<Long, Long> entry1Ids =
			(Map<Long, Long>)portletDataContext.getNewPrimaryKeysMap(
				stagedAssetLink.getEntry1ClassName());
		Map<Long, Long> entry2Ids =
			(Map<Long, Long>)portletDataContext.getNewPrimaryKeysMap(
				stagedAssetLink.getEntry2ClassName());

		long entry1Id = MapUtil.getLong(
			entry1Ids, stagedAssetLink.getEntryId1(), -1);
		long entry2Id = MapUtil.getLong(
			entry2Ids, stagedAssetLink.getEntryId2(), -1);
		...
		_assetLinkLocalService.addLink(
			userId, entry1Id, entry2Id, stagedAssetLink.getType(),
			stagedAssetLink.getWeight());
	}

	protected StagedAssetLink fetchExistingAssetLink(
		long groupId, String entry1Uuid, String entry2Uuid) {

		DynamicQuery dynamicQuery = getAssetLinkDynamicQuery(
			0, groupId, entry1Uuid, entry2Uuid);
	...
	protected DynamicQuery getAssetLinkDynamicQuery(
		long companyId, long groupId, String entry1Uuid, String entry2Uuid) {
		...
		assetEntry1DynamicQuery.add(classUuidProperty.eq(entry1Uuid));
		...
		assetEntry2DynamicQuery.add(classUuidProperty.eq(entry2Uuid));
	...
	protected String parseEntry1Uuid(String uuid) {
		return uuid.substring(0, uuid.indexOf(StringPool.POUND));
	}

	protected String parseEntry2Uuid(String uuid) {
