...
boolean liveSiteStaging = (liveGroup.getRemoteStagingGroupCount() > 0) && PropsValues.STAGING_LIVE_SITE_STAGING_ENABLED;
...
<c:if test="<%= liveSiteStaging %>">
	<div class="alert alert-info">
		<liferay-ui:message key="live-site-staging-alert" />
	</div>
</c:if>
...
<c:choose>
	...
	<c:when test="<%= GroupPermissionUtil.contains(permissionChecker, liveGroup, ActionKeys.MANAGE_STAGING) %>">
		...
		<div class="<%= ((liveGroup.isStaged() || (stagingType != StagingConstants.TYPE_NOT_STAGED)) ? StringPool.BLANK : "hide") %> staging-section" id="<portlet:namespace />stagedPortlets">
			...
			<c:if test="<%= !liveGroup.isCompany() && !liveSiteStaging %>">
			...
			<aui:fieldset helpMessage="staged-portlets-help" label="staged-content">
				...
				%>
					...
					<aui:input disabled="<%= liveSiteStaging %>" label="<%= PortalUtil.getPortletTitle(curPortlet, application, locale) %>" name="<%= StagingUtil.getStagedPortletId(curPortlet.getRootPortletId()) %>" type="checkbox" value="<%= staged %>" />
