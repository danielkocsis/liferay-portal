...
public class LayoutLarPersistenceImpl extends BaseLarPersistenceImpl<Layout>
	...
	public void doDigest(
			Layout layout, LarDigest digest,
		...
		if (layout.isIconImage()) {
			...
			if (image != null) {
				...
				imageLarPersistence.digest(image, digest, portletDataContext);
			}
		}
		...
		if (layout.isTypeArticle()) {
			exportJournalArticle(layout, digest, portletDataContext);
		}
			...
			if (linkToLayoutId > 0) {
				...
				digest(linkedToLayout, digest, portletDataContext);
			}
		}
			...
			for (String portletId : layoutTypePortlet.getPortletIds()) {
				...
				if (portletId.equals("28")) {
					...
					bookmarksPortletLarPersistence.digest(
						portlet, digest, portletDataContext);
				}
			}
		}
	}
	...
	private void exportJournalArticle(
		...
		journalArticleLarPersistence.digest(
			article, digest, portletDataContext);
	}
