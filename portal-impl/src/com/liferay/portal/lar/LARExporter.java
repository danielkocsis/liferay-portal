...
public class LARExporter {
	...
	public void export(
		...
		try {
			...
			context.getZipWriter().getFile();

			_larDigestFile = context.getLarDigest().getDigestFile();
		}
		catch (Exception e) {
			_log.error(e);
		}
	...
	protected void doCreateDigest(
		...
		PortletDataContext _portletDataContext = new PortletDataContextImpl(
			companyId, groupId, parameterMap, new HashSet<String>(),
			context.getStartDate(), context.getEndDate(), zipWriter);
		...
		if (_portletDataContext.hasDateRange()) {
			metadata.put(
				"start-date",
				String.valueOf(_portletDataContext.getStartDate()));
			metadata.put(
				"end-date", String.valueOf(_portletDataContext.getEndDate()));
		}
		...
		_portletDataContext = new PortletDataContextImpl(
			companyId, groupId, parameterMap, new HashSet<String>(),
			context.getStartDate(), context.getEndDate(), null);
