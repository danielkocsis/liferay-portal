							...
							<aui:nav-item cssClass="row-fluid">
								<c:choose>
									<c:when test="<%= (group.isStagingGroup() || group.isStagedRemotely()) && branchingEnabled %>">
										...
										<liferay-staging:staging-menu cssClass="branching-enabled span4" extended="<%= false %>" layoutSetBranchId="<%= layoutRevision.getLayoutSetBranchId() %>" onlyActions="<%= true %>" />
									</c:when>
									...
									<c:otherwise>
										...
										<c:if test="<%= group.isStagingGroup() || group.isStagedRemotely() %>">
											<liferay-staging:staging-menu cssClass="publish-link" extended="<%= false %>" onlyActions="<%= true %>" />
										</c:if>
									</c:otherwise>
								</c:choose>
							</aui:nav-item>
